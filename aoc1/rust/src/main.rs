...
fn main() {
    if let Ok(string) = read_to_string("input.txt") {
        ...
        for i in vec {
            ...
            let first_int_pos = i.find(char::is_numeric).unwrap();
            let last_int_pos = i.rfind(char::is_numeric).unwrap();
            ...
            println!("{i}");
            for (index, strs) in num_strs.clone().iter().enumerate() {
